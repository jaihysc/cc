...
void symtab_pop_cat(Symtab* stab, SymbolCat cat) {
	ASSERT(stab != NULL, "Symtab is null");
	ASSERTF(vec_size(&stab->cat[cat]) > 0, "No symbol in symbol category %d to pop", cat);
...
Symbol* symtab_last_cat(Symtab* stab, SymbolCat cat) {
	ASSERT(stab != NULL, "Symtab is null");
	ASSERTF(vec_size(&stab->cat[cat]) > 0, "No symbol on symbol category %d stack", cat);
...
ErrorCode symtab_push_scope(Symtab* stab) {
	...
	if (stab->scopes_size >= stab->scopes_capacity) {
		...
		Symtab_Scope* new_buf = cmalloc((size_t)stab->scopes_capacity * sizeof(Symtab_Scope));
...
static ErrorCode symtab_add_scoped(Symtab* stab, Symbol** sym_ptr, int i_scope, const char* token, Type* type) {
	ASSERT(stab != NULL, "Symtab is null");
...
ErrorCode symtab_add(Symtab* stab, Symbol** sym_ptr, const char* token, Type* type) {
	ASSERT(stab != NULL, "Symtab is null");
...
ErrorCode symtab_add_label(Symtab* stab, Symbol** sym_ptr) {
	...
	ErrorCode ecode;
	if ((ecode = symtab_add_scoped(stab, sym_ptr, 1, token, &stab->type_label)) != ec_noerr) return ecode;
	symbol_set_valcat(*sym_ptr, vc_nlval);
	return ec_noerr;
}
