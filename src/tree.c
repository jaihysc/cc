...
static ErrorCode do_remove(TNode* node, int* i) {
	...
	if (child->child_count == 0) {
		...
		for (int j = 1; j < child->child_count; ++j) {
			if ((ecode = tnode_attach(node, child->child[j])) != ec_noerr) return ecode;
		}
...
static void debug_tnode_walk(Tree* tree, TNode* node, char* branch, int i_branch, const int max_branch_len) {
	ASSERT(tree != NULL, "Tree is null");
	...
	for (int i = 0; i < node->child_count; ++i) {
		...
		if (i_branch + 2 >= max_branch_len) {
			...
			branch[i_branch + 1] = ' ';
			branch[i_branch + 2] = '\0';
			debug_tnode_walk(tree, node->child[i], branch, i_branch + 2, max_branch_len);
		}
